...
以下是下一阶段计划，不代表当前 Demo 已完成上线能力。排序按“先让作品集可验证，再推进生产化”的优先级组织。

**1. 公开 Demo 展示完善**

- 补齐首页、陪伴对话、情绪日记、长期记忆管理、饮食记录和响应式布局截图。
- 录制 60-90 秒演示视频，重点展示“表达 -> 陪伴回复 -> 保存日记 -> 提炼记忆 -> 后续召回”的闭环。
- 保持 GitHub Pages Demo 可访问，定期检查 GitHub Actions 构建状态和公开链接可用性。

**2. AI 能力可靠性**

- 在后端继续完善 provider registry，让陪伴回复、情绪日记整理、长期记忆提炼和伙伴形象生成都能按能力切换 provider。
- 增加 AI 输出 schema 校验、失败降级、风险分级和成本监控，避免模型输出直接破坏前端状态。
- 建立 Prompt 版本管理和小样本评估集，用固定输入对比回复质量、记忆提炼质量和召回相关性。

**3. 长期记忆结构化升级**

- 将当前本地 `memoryNotes` 升级为带分类、来源、更新时间、置信度和过期策略的结构化记忆。
- 保留用户确认、编辑、删除和纠错入口，避免长期记忆变成不可控黑盒。
- 优化召回策略，只把当前对话真正相关、低敏、有限数量的记忆传给 AI。

**4. 生产后端与账号体系**

- 接入真实短信供应商，完成手机号登录、刷新令牌轮换、退出登录和账号删除的生产联调。
- 补齐 PostgreSQL 持久化环境、迁移执行流程、密钥轮换、限流和隐私日志规则。
- 明确 Demo Token、固定响应、真实认证和生产系统的隔离边界，防止演示配置进入正式环境。

**5. 本地优先云同步**

- 先同步用户画像、伙伴档案、情绪日记和长期记忆，再评估饮食与体重数据是否进入第二阶段。
- 实现离线 outbox、幂等 mutation、服务端版本号、冲突处理和墓碑删除。
- 用契约测试和端到端测试验证断网、重试、重复提交、跨设备恢复和账号删除后的数据边界。

**6. iOS 内测与真实用户验证**

- 补齐完整 Xcode、CocoaPods、签名、隐私说明、TestFlight 和基础监控。
- 小范围验证记忆保存率、有效召回率、日记生成率和用户“被理解感”。
- 根据反馈决定是否继续扩展饮食复盘、伙伴形象生成或更多陪伴场景；新增功能必须服务于核心陪伴闭环。
